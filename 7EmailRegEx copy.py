...
def main():
    ...
    # -------------------------------------------------------
    #  EMAIL TEMPLATE NAME VALIDATION (80 characters)
    # -------------------------------------------------------
    long_names = []
    ...
    for f in files:
        try:
            tree = ET.parse(f)
            root = tree.getroot()
            full_name_el = root.find(f".//{{{NS_URI}}}fullName")

            if full_name_el is not None:
                template_name = (full_name_el.text or "").strip()
            else:
                template_name = ""

            if len(template_name) > 80:
                long_names.append((f.name, template_name, len(template_name)))

        except Exception as e:
            print(f"Error reading {f.name}: {e}")

    if long_names:
        print("\n❌ ERROR: One or more email template names exceed Salesforce's 80 character limit.\n")
        print("Please shorten the following template names:\n")

        for fname, tname, length in long_names:
            print(f"  - File: {fname}")
            print(f"    Template Name ({length} chars): {tname}\n")

        print("➡️  Fix the names in the <fullName> tag inside each template before running again.\n")
        return  # Halt script
