...
def update_page_layout(custom_field_names, layout_path):
    ...
    # Find or create a section to append fields
    detail_sections = root.findall(f".//{{{ns}}}detailLayoutSections")
    if not detail_sections:
        print("[⚠️] No <detailLayoutSections> found in layout.")
        return

    target_section = detail_sections[-1]  # Add to last section (or you could add to a new one)

    for field in custom_field_names:
        layout_item = ET.Element(f"{{{ns}}}layoutItems")
        layout_item.text = field
        target_section.append(layout_item)
